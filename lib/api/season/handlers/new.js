...
const internals = {
   ...
   * @return {Object}
   */
  buildSchedule: (player, ids, clone) => {
    const idsWithoutCurrent = ids.filter(e => e !== player.id);
    const currentPlayer = clone(player);

    idsWithoutCurrent.forEach(e =>
      currentPlayer.schedule.push({ home: currentPlayer.id, away: e })
    );
...
module.exports = {
  ...
  handler: function newSeasonHandler(req, reply) {
    ...
    const ids = [];
    players.forEach(e => ids.push(e.id));
    ...
    const season = {
      ...
      seasonId: uuidv4(),
      players: players
        .map(player => internals.buildSchedule(player, ids, _.clone))
    };
